...
//       return res.status(200).json({ totalExams: exam });
...
router.get('/exams/:id', async (req, res) => {
    try {
        ...
        // Count how many exams this examinee has attempted
        const examCount = await ExamAttempted.countDocuments({ examineeId });

        // Return the count (0 भी आए तो ठीक है, error नहीं)
        return res.status(200).json({ totalExams: examCount });
